...
MANIFEST = json.loads((ROOT.parent / "source-build.json").read_text())
...
def main():
    ...
    if base["revision"] != MANIFEST["revision"]:
    ...
    patches = [
        ...
        if patch["file"]
        in (
            "patches/cef-media-input.patch",
            "media-input-prototype/chromium-tests.patch",
        )
    ]
    for patch in patches:
        path = ROOT.parent / patch["file"]
        directory = cef if patch.get("repository") == "cef" else chromium
    ...
    if args.stage in ("build", "test"):
        ...
        target = (
            "libcef" if args.stage == "build" else "cef_external_video_source_unittests"
        )
        run(
            [work / "depot_tools/ninja", "-C", output, f"-j{args.jobs}", target],
        ...
        if args.stage == "test":
            run(
                [
                    output / target,
                    "--gtest_filter=ExternalVideoSourceTest.*",
    ...
    shutil.copytree(
        args.baseline_runtime.resolve(),
        runtime,
        ignore=shutil.ignore_patterns(
            "libcef.so", "miximus-source-build.json", ".staged"
        ),
    )
    ...
    for path in runtime.iterdir():
        if (
            path.name == "libcef.so"
            or path.is_dir()
            or path.suffix in (".pak", ".bin", ".dat")
        ):
            (link / path.name).symlink_to(Path("../runtime") / path.name)
    provenance = dict(
        MANIFEST, libcef_sha256=sha(runtime / "libcef.so"), experimental=True
    )
    (runtime / "miximus-media-input-prototype.json").write_text(
