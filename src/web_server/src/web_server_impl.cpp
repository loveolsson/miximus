...
web_server_impl::web_server_impl() {
  ...
  endpoint_.set_message_handler(
      bind(&web_server_impl::on_message, this, _1, _2));
}

web_server_impl::~web_server_impl() { stop(); }

void web_server_impl::terminate_and_log(connection_hdl hdl,
...
void web_server_impl::on_message(connection_hdl hdl, message_ptr msg) {
  ...
  case message::action_t::subscribe: {
    ...
    if (topic != message::topic_t::invalid) {
      ...
      response = message::create_error_base_payload(
          token, message::error_t::invalid_topic);
    }
  ...
  case message::action_t::error: {
    auto topic = message::get_topic_from_payload(json);
    if (topic > message ::topic_t::invalid &&
        topic < message::topic_t::_count && subscriptions_[(int)topic]) {
      subscriptions_[(int)topic](action, std::move(json), con->second.id);
    } else {
      auto token = message::get_token_from_payload(json);
      endpoint_.send(hdl,
                     message::create_error_base_payload(
                         token, message::error_t::internal_error)
                         .dump(),
                     opcode::text);
    }
  } break;
...
void web_server_impl::on_open(connection_hdl hdl) {
  endpoint_.get_alog().write(websocketpp::log::alevel::app,
                             "Connection opened");
  auto &con =
      connections_.emplace(hdl, websocket_connection{next_connection_id++, {}});
...
void web_server_impl::subscribe(message::topic_t topic,
                                message::callback_t callback) {
  endpoint_.get_io_service().post(
      [this, topic, callback]() { subscriptions_[(int)topic] = callback; });
}
...
void web_server_impl::stop() {
  ...
  endpoint_.get_io_service().post([&]() {
    ...
    for (auto it = connections_.begin(); it != connections_.end(); it++) {
      ...
      endpoint_.close(it->first, websocketpp::close::status::going_away,
                      "Server shutting down", ec);
      if (ec) {
...
void web_server_impl::send_message(const nlohmann::json &msg,
                                   int64_t connection_id) {
  auto topic = message::get_topic_from_payload(msg);
  auto serialized = msg.dump();
  endpoint_.get_io_service().post(
      [this, topic, serialized = std::move(serialized), connection_id]() {
        send_message_sync(topic, serialized, connection_id);
      });
}

void web_server_impl::send_message_sync(const nlohmann::json &msg,
                                        int64_t connection_id) {
  auto topic = message::get_topic_from_payload(msg);
  auto serialized = msg.dump();
  send_message_sync(topic, serialized, connection_id);
...
void web_server_impl::send_message_sync(message::topic_t topic,
                                        const std::string &msg,
                                        int64_t connection_id) {
  ...
  if (con->second.topics.count(topic)) {
...
void web_server_impl::broadcast_message(const nlohmann::json &msg) {
  ...
  auto serialized = msg.dump();
  endpoint_.get_io_service().post(
      [this, topic, serialized = std::move(serialized)]() {
...
void web_server_impl::broadcast_message_sync(message::topic_t topic,
                                             const std::string &msg) {
