...
template <typename T>
constexpr auto default_option_setter = [](T& t, const nlohmann::json& j) -> bool {
    try {
        t = j;
...
template <typename T>
constexpr auto default_option_getter = [](const T& t) -> nlohmann::json { return t; };

template <typename T>
class option_typed : public option_i
{
    T                     v_;
    const option_set_f<T> setter_;
    const option_get_f<T> getter_;
