...
void exercise_gpu_failure(gpu::device_s& device, session_s& session)
{
    ...
    auto destination = device.create_texture({.width = 801, .height = 451});
...
void exercise_color(gpu::device_s& device, session_s& session)
{
    ...
    const std::array cases{
        case_s{.css = "rgb(255,0,0)",              .reference = {1, 0, 0, 1},                                            .tolerance = 0.00004F},
        case_s{.css = "rgb(0,255,0)",              .reference = {0, 1, 0, 1},                                            .tolerance = 0.00004F},
        case_s{.css = "rgb(0,0,255)",              .reference = {0, 0, 1, 1},                                            .tolerance = 0.00004F},
        case_s{.css       = "rgb(128,64,32)",
               .reference = {decode(128.F / 255), decode(64.F / 255), decode(32.F / 255), 1},
               .tolerance = 0.00004F                                                                                                          },
        case_s{.css       = "rgba(128,64,32,0.5)",
               .reference = {decode(.5F) * 128 / 255, decode(.25F) * 128 / 255, decode(.125F) * 128 / 255, 128.F / 255},
               .tolerance = 0.003F                                                                                                            },
        case_s{.css = "rgba(255,0,0,0.01)",        .reference = {3.F / 255, 0, 0, 3.F / 255},                            .tolerance = 0.004F  },
        case_s{.css = "transparent",               .reference = {0, 0, 0, 0},                                            .tolerance = 0.00004F},
    };
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        gpu::device_s device(options);
        {
            ...
            auto context   = device.create_recording_context(1);
            auto output    = device.create_texture({.width = 640, .height = 360});
            auto recording = context.try_record();
